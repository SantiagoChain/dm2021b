...
karch_dataset    <- "./datasets/dataset_epic_v002.csv.gz"
...
HemiModelos  <- function( hparam )
{
  ...
  particionar( dataset, division= c(50,50), agrupa= c("foto_mes","clase_ternaria" ) )

  #Construyo el modelo sobre el fold=1
  dgeneracion1  <- lgb.Dataset( data=    data.matrix(  dataset[ generacion==1 & fold==1, campos_buenos, with=FALSE]),
                                label=   dataset[ generacion==1 & fold==1, clase01] )
  ...
  prediccion  <- predict( modelo_final1, data.matrix( dataset[ generacion==1 & fold==2, campos_buenos, with=FALSE]) )
  dataset[ generacion==1 & fold==2, prob := prediccion ]

  tb_modelitos[ dataset[ generacion==1 & fold==2], 
  ...
  dgeneracion2  <- lgb.Dataset( data=    data.matrix(  dataset[ generacion==1 & fold==2, campos_buenos, with=FALSE]),
                                label=   dataset[ generacion==1 & fold==2, clase01]
                              )
  ...
  prediccion  <- predict( modelo_final2, data.matrix( dataset[ generacion==1 & fold==1, campos_buenos, with=FALSE]) )
  dataset[ generacion==1 & fold==1, prob := prediccion ]

  tb_modelitos[ dataset[ generacion==1 & fold==1], 
                on= c("numero_de_cliente","foto_mes"),  
                paste0( "E", kexperimento,"_", GLOBAL_iteracion ) := i.prob  ]
...
FullModelo  <- function( hparam )
{
  ...
  dgeneracion  <- lgb.Dataset( data=    data.matrix(  dataset[ generacion==1 , campos_buenos, with=FALSE]),
...
campos_buenos  <- setdiff( colnames(dataset), 
                           c("clase_ternaria","clase01", "generacion", "entrenamiento", "validacion",campos_malos) )
